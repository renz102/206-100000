...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    foreach ($items as $item) {
        ...
        $stmt->bind_param("ssssis", $fullname, $email, $address, $paymentMethod, $user_id, $product_name);
        $stmt->execute();
    }
...
function proceedToPayment() {
  ...
  checkedItems.forEach(check => {
    ...
    const name = div.querySelectorAll('span')[0].innerText;
    const price = parseFloat(div.querySelectorAll('span')[1].innerText.replace(/[^0-9.-]+/g,"").replace(/,/g,""));
